...
fulldisclosure_range = list(range(0,4))
hackerone_c_range = list(range(0,7))
hackerone_hacktivity_range = list(range(0,5))
hackerone_public_range = list(range(0,4))
netsec_range = 8
stackoverflow_range = 7
twitter_range = list(range(0,2))
...
saved = template.render(date=date_now, newsletter_id=timestamp, bugcrowd_data=bugcrowd_data, bugcrowd_notations=bugcrowd_notations, fulldisclosure_data=fulldisclosure_data, 
...
online_view_save = open("../static/newsletters/" + str(timestamp) + ".html", "w")
online_view_save.write(saved)
online_view_save.close()
